...
#define IDC_BUTTON1             30
#define IDC_BUTTON2             31
#define IDC_BUTTON3             32
#define IDC_BUTTON4             33
#define IDC_BUTTON5             34
#define IDC_FORWARD             35
#define IDC_BACKWARD            36
...
void InitApp()
{
 ...
 //   g_HUD.AddButton( IDC_CHANGEDEVICE, L"Change device (F2)", 35, iY += 24, 125, 22, VK_F2 );
    ...
    g_SampleUI.SetFont( 1, L"Comic Sans MS", 12, FW_NORMAL );
	//g_SampleUI.SetFont( 2, L"Courier New", 12, FW_NORMAL );

 //   // Static
 //   g_SampleUI.AddStatic( IDC_STATIC, L"This is a static control.", 0, 0, 200, 30 );
	////

 //   g_SampleUI.AddStatic( IDC_OUTPUT, L"this is static output.", 20, 50, 620, 300 );
 ...
 //   // Buttons
    //g_SampleUI.AddButton( IDC_ENABLEIME, L"Draw Text", 430, 390, 80, 35, L'D' );
	g_SampleUI.AddButton( IDC_BUTTON1, L"1", 200, 390, 50, 35, L'1' );
	g_SampleUI.AddButton( IDC_BUTTON2, L"2", 250, 390, 50, 35, L'2' );
    g_SampleUI.AddButton( IDC_BUTTON3, L"3", 300, 390, 50, 35, L'3' );
	g_SampleUI.AddButton( IDC_BUTTON4, L"4", 350, 390, 50, 35, L'4' );
    g_SampleUI.AddButton( IDC_BUTTON5, L"5", 400, 390, 50, 35, L'5' );

	g_SampleUI.AddButton( IDC_FORWARD, L"<", 500, 390, 50, 35, L'<' );
	g_SampleUI.AddButton( IDC_BACKWARD, L">", 550, 390, 50, 35, L'>' );

 //   // Edit box
    g_SampleUI.AddEditBox( IDC_EDITBOX1, L"", 20, 440,
                           600, 32 );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		CDXUTEditBox* editBox1 = g_SampleUI.GetEditBox(IDC_EDITBOX1);
		CDXUTButton* button1 = g_SampleUI.GetButton(IDC_ENABLEIME);
		CHAR strFileName[128];
		

		WideCharToMultiByte(CP_ACP, NULL, editBox1->GetText(), -1, strFileName,  128, NULL, FALSE);
		CHAR strFileNameWithExt[128];
		strcpy_s(strFileNameWithExt,strFileName);
		strcat_s(strFileNameWithExt,".txt");
		if(g_Script.ReadFromFile(strFileNameWithExt))
		{
			char* pdest = NULL;
			int  result = 0;
			pdest = strstr(strFileNameWithExt,"p");
			result = (int)(pdest - strFileNameWithExt + 1);
			char c = strFileNameWithExt[result];
			g_iAnswer = atoi(&c);
		}
		g_Scene.Draw(pd3dDevice,g_Script.GetReadBuffer(),g_fAngle);
		if(g_bAnswerCorrect)
		{
			g_fAngle++;
			if(g_fAngle>360.0f)
...
void RenderText()
{
	...
	WCHAR strWC[1024];
	MultiByteToWideChar( CP_ACP, 0, g_Script.GetReadBuffer(), -1, strWC, 1024 );
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
	if((nChar == 32)&&(bKeyDown == TRUE)) // 스페이스
	{
		//g_Script.NextLine();
	}
}
